...
func ToCommandProto(v any) *commandeventspb.CommandEventValue {
	if v == nil {
		return nil
	}
	switch val := v.(type) {
...
func ToSnapshotProto(v any) (*snapshotpb.Value, error) {
	switch val := v.(type) {
	case string:
...
func FromSnapshotProto(v *snapshotpb.Value) any {
	if v == nil {
		return nil
	}
